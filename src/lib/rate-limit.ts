...
export async function checkRateLimit(
  ...
  // Reset count if window_start is from a previous day
  await query(
    `INSERT INTO rate_limits (fingerprint_hash, action_type, model_id, count, window_start)
     VALUES (?, ?, ?, 1, NOW())
     ON DUPLICATE KEY UPDATE
       count = CASE
         WHEN DATE(window_start) < CURDATE() THEN 1
         WHEN count < ? THEN count + 1
         ELSE count
       END,
       window_start = CASE
         WHEN DATE(window_start) < CURDATE() THEN NOW()
         ELSE window_start
       END`,
    [fingerprintHash, action, effectiveModelId, maxRequests]
  );
